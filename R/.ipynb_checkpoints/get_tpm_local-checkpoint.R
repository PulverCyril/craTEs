get_tpm_local <- function(countTable, org='hg19', exonic_lengths_path=NULL, redownload_exons=FALSE) {
    ...
    ## Description: computes TPM from a count table
        ## Para: -countTable: table with ensembl gene IDs as row names, sample IDs as columns
                #- org: organism (defaults to 'hg19')
                #- exonic_lengths_path: path to where computed exonic lengths are stored, to avoid recomputing them everytime
                #- redownload_exons: forces the download and computation of exon lengths from scratch
